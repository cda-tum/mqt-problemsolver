...
def assert_returns_0(cost_function: cf.CostFunction, path: dict[sp.Expr, int]) -> None:
    settings = cf.PathFindingQUBOGeneratorSettings(cf.EncodingType.ONE_HOT, 1, TEST_GRAPH.n_vertices, False)
...
def assert_returns_gt_0(cost_function: cf.CostFunction, path: dict[sp.Expr, int]) -> None:
    settings = cf.PathFindingQUBOGeneratorSettings(cf.EncodingType.ONE_HOT, 1, TEST_GRAPH.n_vertices, False)
    formula = cost_function.get_formula(TEST_GRAPH, settings)
    value = formula.subs(path).doit()  # type: ignore[no-untyped-call]
    assert value > 0
