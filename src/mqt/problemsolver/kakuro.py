...
class Kakuro:
    ...
    def init_qc(self):
        ...
        qc = QuantumCircuit(a_low, a_high, b_low, b_high, c_low, c_high, d_low, d_high, anc, anc_mct, flag)
        ninputs = 8
        nqubits = ninputs + 1
    ...
    def simulate(self, qc):
        ...
        if found_sol:
            for entry in counts.keys():
                ...
                a = int(entry[6:8], 2)
                if counts.get(entry) > 5 * mean_counts:
                    self.print_kakuro_solved(a, b, c, d, self.s0_input, self.s1_input, self.s2_input, self.s3_input)
        else:
